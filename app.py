...
APPLICATION_NAME = "E-Furniture Application"


# Connect to Database and create database session
engine = create_engine('sqlite:///efurniture.db?check_same_thread=False')
...
def gconnect():
    ...
    if stored_access_token is not None and gplus_id == stored_gplus_id:
        response = make_response(json.dumps('CurrentUser, already connected.'),
                                 200)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    output += '-webkit-border-radius: 150px;-moz-border-'
    output += 'radius: 150px;"> '
...
def gdisconnect():
    ...
    if result['status'] == '200':
        ...
        del login_session['user_id']
        login_session.clear()
        response = make_response(json.dumps('Successfully logedOff.', 400))
        response.headers['Content-Type'] = 'application/json'
        return response
...
def newItem(category_id):
    ...
    if request.method == 'POST':
        newItem = Item(name=request.form['name'], description=request.form[
                       ...
                       category_id=category_id, userid='user_id')
        session.add(newItem)
        session.commit()
...
def editItem(category_id, item_id):
    ...
    category = session.query(Category).filter_by(id=category_id).one()
    count = 1000
    if editedItem.userid != 'user_id':
        flash('Sorry, Item is not belongs to you!')
        return redirect(url_for('showMenu', category_id=category_id))
    if request.method == 'POST':
...
def deleteItem(category_id, item_id):
    ...
    if itemToDelete.userid != 'user_id':
        flash('Sorry, Item is not belongs to you!')
        return redirect(url_for('showMenu', category_id=category_id))
    if request.method == 'POST':
